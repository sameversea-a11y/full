  ...
  async uploadFiles(req, res) {
    try {
      ...
      const metadata = {
        ...
        customerInfo: req.body.customerInfo ? JSON.parse(req.body.customerInfo) : null,
        pricingSnapshot: req.body.pricingSnapshot ? JSON.parse(req.body.pricingSnapshot) : null,
      ...
      for (let i = 0; i < req.files.length; i++) {
        ...
        try {
          ...
          const fileMetadata = {
            documentTypeId: req.body[`fileMetadata[${i}][documentTypeId]`] || '',
          ...
          if (existingDoc) {
            ...
            fs.unlinkSync(file.path);
            errors.push(`File "${file.originalname}" has already been uploaded`);
            continue;
          }
          ...
          errors.push(`Failed to process file "${file.originalname}": ${error.message}`);
        }
      }
